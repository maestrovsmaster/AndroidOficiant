 ...
 */
public class Surface implements Telo{
   ...
   // private int[] textureIDs = new int[numFaces];
    ...
    public Surface(Context context, Coord3d coord3d, float widhtX, float lenthZ, int koef, int textureResId) {
    ...
    private void calculateVertexes() {
       ...
       for(int iz=0;iz<numF;iz++)
       {
           ...
           for(int ix=0;ix<numF;ix++){
               ...
               float y1=Y+surfList.get(iz).get(ix);
               float y2=Y+surfList.get(iz).get(ix+1);
               ...
               float y3=Y+surfList.get(iz+1).get(ix);
               float y4=Y+surfList.get(iz+1).get(ix+1);
